...
\section{Bloc B --- Estructura Científica (6 punts)}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3cm}YYYY}
    ...
    B3 Conclusions crítiques \emph{(1.25 pt)} & Inclou limitacions del tema, preguntes obertes i una subsecció explícitament titulada «Reflexió Personal»; va clarament més enllà del resum & Dos dels tres elements crítics presents; la reflexió personal hi és però no té subsecció pròpia amb títol & Conclusions presents però bàsicament un resum; la reflexió personal és mínima o no apareix com a subsecció diferenciada & Conclusions absents, purament descriptives o sense cap reflexió personal \\
    B4 Elements de contingut \emph{(1.0 pt)} & Mínim 2 taules i 2 figures, totes referenciades al text amb \texttt{\textbackslash ref\{\}}, totes les cites inline presents i totes les figures/taules inclouen la indicació de la font (p.ex.\ \textit{Font: Compilada per l'Autor}) & Taules i figures presents però alguna sense cross-reference o sense indicació de font & Taules o figures per sota del mínim, o moltes sense referenciar o sense indicació de font & Taules i figures absents, cap cross-reference o cap indicació de font \\
    ...
    B7 Notes al peu \emph{(1.0 pt)} & Mínim 3 notes al peu (\texttt{\textbackslash footnote\{\}}), rellevants i substantives, ben distribuïdes al llarg del document & 3 notes presents però alguna trivial o mal col·locada & 1--2 notes al peu o notes molt breus sense contingut & Cap nota al peu present \\
    \bottomrule
  \end{tabularx}
  }
\end{table}
...
\medskip
\textbf{Nota sobre la indicació de font:} Per a figures i taules de creació pròpia, s'ha d'incloure la línia \textit{Font: Compilada per l'Autor} just sota la llegenda (\texttt{\textbackslash caption}). Si la figura o taula prové d'una font externa, cal citar-la amb la referència bibliogràfica corresponent (p.ex.\ \textit{Font: adaptat de \textbackslash cite\{key\}}).

\section{Bloc C --- Execució LaTeX (2 punts)}
...
El professor pot atorgar manualment entre 0.5 i 1.0 punts addicionals a treballs que destaquin clarament per sobre de la resta. Els criteris per atorgar-los són subjectius i exclusius del professor: un tema especialment atractiu o original, un pensament crític genuí i ben argumentat, o unes qualitats STEM (rigor, anàlisi quantitativa, connexions interdisciplinàries) que cap altre treball del grup no presenta de manera comparable. Aquests punts extra no estan garantits i no es poden reclamar.
...
\[
  ...
  \left(\frac{\mathrm{Bloc\ B}}{6}\right)\times 6 +
  \left(\frac{\mathrm{Bloc\ C}}{2}\right)\times 2 +
  \mathrm{Punts\ extres}
\]
...
\begin{table}[htbp]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{10cm}c}
    ...
    Bloc B --- Estructura Científica & 6.0 \\
    Bloc C --- Execució LaTeX & 2.0 \\
