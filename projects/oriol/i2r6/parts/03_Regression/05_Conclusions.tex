...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \textbf{Small Sample ($N \le 100$)} & Parametric (Normal) or CART & Parametric (LogReg) \\
...
The influence of sample size is clearest in the logistic results. Figures~\ref{fig:logistic-cio-univariate} and~\ref{fig:logistic-heatmap} show that CIO deteriorates most visibly when $N=100$, whereas the linear results show a much weaker sample-size pattern.

\begin{itemize}
    \item \textbf{High N, Low p:} CART and Parametric are both viable.
    \item \textbf{Small N:} The strongest caution applies to logistic regression, where Figures~\ref{fig:logistic-cio-univariate} and~\ref{fig:logistic-heatmap} show a clear deterioration at $N=100$; for linear regression, the sample-size effect is much less pronounced.
...
\begin{itemize}
    ...
    \item \textbf{Metric Focus:} We focused on CIO, Bias Ratio, and CI Width Ratio. Other metrics (e.g., coverage, MSE) might reveal different tradeoffs.
\end{itemize}
