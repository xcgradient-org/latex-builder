...
The coefficients are fixed at $\beta_j = (-1)^j$ to ensure a mix of positive and negative relationships. The outcome $Y$ is then generated from the corresponding model given the predictor matrix: for linear regression, from the linear predictor $\mathbf{X}\beta$ plus a Gaussian stochastic component; for logistic regression, from the Bernoulli distribution induced by the logistic transformation of $\mathbf{X}\beta$. The same data-generation logic is encoded in the regression configuration summarized in Appendix~\ref{sec:app-json}.
...
That is, the metric directly compares the inferential intervals obtained from the original fit and the corresponding synthetic fit.
