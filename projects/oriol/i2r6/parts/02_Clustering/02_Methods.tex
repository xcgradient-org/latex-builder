...
\textbf{Mathematical Formulation (Ward's Linkage)}
...
\begin{itemize}
  \item \textbf{Input:} A set of $N$ vectors $X \subset \mathbb{R}^d$. Initially, each point represents a singleton cluster, resulting in $N$ clusters.
  \item \textbf{Error Sum of Squares (ESS):} For any cluster $C$, the variance is defined as the sum of squared Euclidean distances from the points in $C$ to its centroid $\mu_C$:
  \begin{equation}
    \text{ESS}(C) = \sum_{x \in C} ||x - \mu_C||^2
  \end{equation}
  \item \textbf{Objective Function (Step-wise):} At each step, the algorithm finds and merges the pair of clusters $(A, B)$ that results in the minimum increase in the total ESS. The distance (or merging cost) between $A$ and $B$ is exactly this increase:
  \begin{equation}
    D(A, B) = \text{ESS}_{A \cup B} - (\text{ESS}_A + \text{ESS}_B)
  \end{equation}
  \item \textbf{Output:} A dendrogram (tree) representing the sequence of merges, which can be cut at a specific depth to yield $k$ clusters.
\end{itemize}
