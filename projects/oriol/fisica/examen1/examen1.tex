...
\begin{center}
  \vspace*{-15pt}
  {\Huge\bfseries Examen de Cinemàtica}\\[8pt]
...
L'octubre de 1927, durant la cèlebre Cinquena Conferència de Solvay a Brussel·les, Albert Einstein i 
Niels Bohr van protagonitzar un dels debats científics més importants de la història sobre la mecànica 
quàntica. Després d'una sessió esgotadora on Einstein va pronunciar la seva mítica frase ``Déu no juga 
als daus'', es va enrecordar que tenia el tren cap a Berlín en \textbf{6 minuts exactes}. Va agafar 
la seva bicicleta i va sortir pedant cap a l'estació a \textbf{18 km/h}. L'estació es troba a 
\textbf{150 decàmetres} de l'hotel. Niels Bohr, que estava acabant una conversa amb Heisenberg, 
s'adona \textbf{2 minuts més tard} que Einstein s'ha deixat uns papers confidencials sobre la taula 
amb anotacions crítiques sobre el Principi d'Incertesa. Surt corrent darrere seu a 
\textbf{250 m/minut} per intentar retornar-li'ls abans que el tren marxi.

\begin{enumerate}[label=\textbf{\alph*)}]
  \item Escriu les equacions de posició $x(t)$ per a tots dos físics.
  \item Arribarà Einstein a l'estació abans que surti el tren? Si és que sí, amb quant de temps de marge (en minuts)?
  \item Aconseguirà Bohr retornar-li els papers abans que marxi el tren? Si és que no, quant de temps (en minuts) li faltarà per arribar a l'estació?
...
\begin{enumerate}[label=\textbf{\alph*)}]
  \item Quina ha estat l'acceleració (en m/s²) en cadascun dels dos trams ($a_1$ i $a_2$)?
  ...
  \item Dibuixa les 3 gràfiques ($x-t$, $v-t$, $a-$t) de tot el moviment complet (unint els dos trams), explicant en els eixos el criteri temporal del segon tram: si el fas localment, $t_0=0$; si el fas globalment, $t_0=t_{1f}$.
\end{enumerate}
...
\begin{enumerate}[label=\textbf{\alph*)}]
  \item Escriu les equacions de posició $y(t)$ per a totes dues boles.
...
\begin{enumerate}[label=\textbf{\alph*)}]
  ...
  \item A quina altura màxima, respecte al terra de gespa, tenia pensat en Federer colpejar la pilota ($y_{\max}$) i en quin instant s'assoleix aquest punt ($t_{\max}$)?
  ...
  \item Amb quina velocitat impacta la pilota contra el terra?
  \item Dibuixa les 3 gràfiques ($y$-$t$, $v$-$t$, $a$-$t$) de l'acció completa, explicant-hi els punts i instants rellevants.
\end{enumerate}
