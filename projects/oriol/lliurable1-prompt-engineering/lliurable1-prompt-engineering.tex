...
% Indicació de font sota les llegendes de figures i taules
\newcommand{\source}[1]{\par\vspace{0.15em}{\small\textit{Font: #1.}}}
...
\begin{table}[H]
  ...
  \label{tab:models}
  \source{Compilada per l'Autor a partir de dades públiques dels fabricants}
  \begin{tabular}{llll}
    \toprule
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\small]
    \tikzset{
      techbox/.style={draw, thick, rounded corners=6pt,
                      minimum width=3.6cm, minimum height=2.2cm,
                      align=center, text width=3.2cm}
    }
    % Three technique boxes
    \node[techbox, fill=blue!12] (zs) at (0,0)
      {\textbf{Zero-Shot}\\[5pt] Instrucció directa\\sense exemples previs};
    \node[techbox, fill=teal!12] (fs) at (5,0)
      {\textbf{Few-Shot}\\[5pt] $k$ parells\\entrada\,$\to$\,sortida};
    \node[techbox, fill=orange!15] (cot) at (10,0)
      {\textbf{Chain of Thought}\\[5pt] Passos de raonament\\explícits};
    % Complexity axis
    \draw[->, thick, gray!70] (-2,-1.6) -- (12,-1.6);
    \node[gray!80, right] at (12,-1.6) {\footnotesize complexitat};
    \node[gray!80] at (-2,-2.0) {\footnotesize menys};
    \node[gray!80] at (12,-2.0) {\footnotesize més};
  \end{tikzpicture}
  \caption{Comparació de les tres tècniques principals de prompting}
  \label{fig:tecni}
  \source{Compilada per l'Autor}
\end{figure}
...
Un cas típic és demanar la resolució d'un sistema de dues equacions sense proporcionar cap guia de procés:
\begin{verbatim}
Resol el sistema d'equacions següent.
Dona únicament els valors de x i de y.

  2x + y = 5
   x - y = 1
\end{verbatim}
En aquest exemple l'objectiu és clar (resoldre el sistema) i el format de sortida queda fixat (``x = ?, y = ?''), cosa que redueix ambigüitats sense necessitat d'exemples addicionals. El model aplica directament el coneixement algebraic adquirit durant el preentrenament.
...
L'exemple següent mostra un prompt few-shot aplicat a la resolució de sistemes de dues equacions: el model rep dos sistemes resolts com a demostració i ha de resoldre'n un tercer seguint el mateix patró.\footnote{Estudis mostren que el rendiment del few-shot és sensible al nombre d'exemples k: massa pocs no estableixen el patró, massa molts poden sobrecarregar el context \cite{liu2023}.}
\begin{verbatim}
Resol cada sistema d'equacions. Dona els valors de x i y.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=2.6cm, >=stealth, font=\small]
    \tikzset{
      stepbox/.style={draw, thick, rounded corners=5pt,
    ...
    \node[stepbox] (a) {Exemples \(k\)};
    \node[stepbox, right of=a] (b) {Prompt};
    \node[llmbox,  right of=b] (c) {\textbf{LLM}};
    \node[stepbox, right of=c] (d) {Resposta};
    \draw[->, thick] (a) -- (b);
    \draw[->, thick] (b) -- (c);
    \draw[->, thick] (c) -- (d);
  \end{tikzpicture}
...
\subsection{Exemple comparat: sistema d'equacions sense CoT vs. amb CoT}
Sense CoT, el prompt pot ser: ``Resol el sistema \(2x + y = 5\) i \(x - y = 1\). Quant val \(x\)?'' Amb CoT, es reformula: ``Resol pas a pas el sistema \(2x + y = 5\) i \(x - y = 1\). Mostra cada operació i dona \(x\) i \(y\).'' La segona versió convida el model a sumar les equacions \((2x + y) + (x - y) = 5 + 1\), obtenint \(3x = 6\) i per tant \(x = 2\), i desprès substituir per trobar \(y = 1\). Fer explícit cada pas redueix significativament els errors en problemes algebraics multietapa.
...
\begin{table}[H]
  ...
  \source{Valors il·lustratius compilats per l'Autor}
  \begin{tabular}{lccc}
    \toprule
...
\begin{quote}
\ttfamily\small
Resol el sistema d'equacions i dona la solució.\\
\\
Sistema:\\
\quad 3x + 2y = 12\\
\quad  x -  y =  1\\
...
\\
Exemple 2\\
Sistema: 2x + y = 9 ; x + y = 5\\
Solució: x = 4, y = 1\\
\\
Ara resol:\\
Sistema: 3x + 2y = 12 ; x - y = 1\\
Solució:
\end{quote}
...
\begin{quote}
\ttfamily\small
Resol pas a pas el sistema d'equacions i dona la solució final.\\
\\
Sistema:\\
\quad 2x + y = 5\\
\quad  x - y = 1\\
\\
Mostra:\\
1. Mètode triat (substitució o eliminació)\\
2. Operacions pas a pas\\
3. Solució: x = ?, y = ?
\end{quote}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.3cm}>{\raggedright\arraybackslash}X>{\centering\arraybackslash}p{2.2cm}>{\raggedright\arraybackslash}p{4.2cm}}
    ...
    \midrule
    Zero-Shot & Resoldre un sistema 3x+2y=12; x-y=1 & 4 & Correcte en casos clars; errors en sistemes amb coeficients no enters \\
    Few-Shot & Resoldre un sistema a partir de 2 exemples resolts & 5 & El patró dels exemples orienta eficaçment la resolució \\
    CoT & Resoldre 2x+y=5; x-y=1 pas a pas amb mètode explicitat & 5 & Raonament transparent; errors aritmètics molt reduïts \\
    \bottomrule
  \end{tabularx}
\end{table}
